...
NEXT PASS: (1) **READ THE 09-23 01:20Z DIVERGENCE RUN** -- it must print `reported ... but by DIFFERENT code ... re-deriving` and produce the first `report_code`-stamped artifact (the one this pass wrote is unstamped, by construction: it ran before the fix). That is the end-to-end proof of #76, and it costs 9.5 min of batch time. (2) **panel day 09-23, run at 8 of 10**. (3) `Economics|1h|d2` atlas re-read >= ~2 weeks out. (4) weather width-24 reading two 09-30. (5) econ width-24 bracket on EVENT rollover, late Sept. Gated items unchanged from the 02:45Z entry below.
