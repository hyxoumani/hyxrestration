...
Updated: **2026-09-23 14:40Z (THE 09-21 SPLIT GAVE ITS BUDGET FRACTION TO THE RETRY CLASS THAT FIRED TWICE IN 48h AND A BARE COUNT TO THE ONE THAT FIRED NINETEEN TIMES -- AND THAT COUNT CANNOT SAY WHETHER A CYCLE WAS ONE 429 FROM DYING. SHIPPED, NO PROMOTE YET.)**
...
**NOT PROMOTED THIS PASS -- and that is a decision, not an omission.** `collector/venues/kalshi.py` is in the stream and shadow daemon closures, and `hyxlab-shadow` is deliberately pinned to old code until 09-27 to keep the only run ever to bank 10 panel days alive to 14 (02:40Z entry). Promoting now either restarts it or installs a closure its running process does not match. **Queued behind the 09-27 panel re-read, with the shadow restart.** `hyxlab-breadth` is a 5-minute oneshot and picks the change up on its first firing after the promote, so the reading costs nothing to wait for.
NEXT PASS: (1) **read the 09-24 06:10Z sweep's `attach_wait` split** -- `contended_n`, `slept_s_total` vs `open_s_total`, first production reading of #78. (2) **re-read the panel 09-27 at 14 days**, and promote #79 + restart shadow behind it. (3) **first `rate_limit_budget_frac` readings** once promoted -- a cycle at 1.0 is one 429 from a lost cycle and the 7 cycles that read `2` this window are the population to watch. (4) `Economics|1h|d2` atlas re-read >= ~2 weeks out. (5) weather width-24 reading two 09-30. (6) econ width-24 bracket on EVENT rollover, late Sept. (7) **the #79 sweep**: which other observable in this repo is shared by ladders with DIFFERENT concurrency units? `sweep.BURST_OPEN_RETRIES` (per burst) and the `_TransportBudget` handed to `get_market_trades` (per request, and currently absent) are the two to ask.
