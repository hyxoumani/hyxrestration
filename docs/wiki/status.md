...
Updated: **2026-09-22 02:45Z (THE DIGEST FLAGGED A BREADTH FAILURE AND IT IS NOT NEW: THE 09-08 RETRY FIX RETRIED A ReadTimeout AND LEFT ITS IDENTICAL TWIN -- A 504 -- FATAL, AND THE THREE INSTANCES FIVE DAYS LATER WERE SEEN AND DISMISSED AS FLOOD SYMPTOMS. SHIPPED AND PROMOTED, NO RESTART.)**
...
**PROMOTED, NO RESTART.** `daemon_imports.py intersect` is EMPTY for all three of `collector.streamd`, `simulator.shadow` and `simulator.simui.__main__`, so nothing restarts and the panel keeps its 8 of 10; `hyxlab-breadth` is a oneshot and picks the fix up on its next firing. Committed and pushed.
NEXT PASS: (1) **panel day 09-23, run at 8 of 10** -- tomorrow. (2) **READ THE PROMOTED-CODE DIVERGENCE RUN** -- it fired 55m before this pass and carries the first production `nearest_qty_delta` (expect `n: 0, equal_qty: 0`; nonzero `equal_qty` at the 2s window falsifies #69) AND the first `attach_wait`, whose shadow row runs the brief-reader default against a daemon-held file, so `budget_frac_max` there is the number to read first. This is now the top unblocked item. (3) **the first `walk_budget_frac` / per-class retry readings from breadth** -- every cycle publishes them from the next firing; a nonzero `gateway` count is the fix working, and a `walk_budget_frac` reaching 1.0 is one blip from failing again. (4) `Economics|1h|d2` atlas re-read >= ~2 weeks out. (5) weather width-24 reading two 09-30. (6) econ width-24 bracket on EVENT rollover, late Sept. (7) first PRODUCTION `shutdown; stats` line -- restart-gated, six deferred changes riding it. (8) `prioritycheck` drift watch, ~2 weeks. (9) `LIVE_GRACE_S` measured-not-re-sized, pending production `tick_gap`. (10) **the #73 sweep**: which other fix in this repo has its predicate keyed on the mechanism that reported the fault rather than the fault? `get_market_trades`/`get_series_list`'s bare `sess.get` is the known instance of the same ladder; the wider question is which `except SomeSpecificError` in the repo names an observation channel instead of a condition.
