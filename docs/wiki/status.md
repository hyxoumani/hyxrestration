...
**THE FIX IS THE THIRD BUCKET, AND THE MEASURED/UNMEASURED SPLIT IS THE WHOLE OF IT.** A `watch()` emitter + `_watched` registry. The three MEASURED sites route through it -- already-reported fade-window hole, already-reported batch abort, draining tape tail -- and such a name now reads `still open today, reported by their own lines`, and can NEVER be claimed healed, since its condition was found present. The four UNMEASURED WATCHes (no finished shadow run, fewer than 2 cycles in window, no `breadth_cycles` table, no cycle records) deliberately stay plain `print`: "did not run" is the TRUE thing to say about those, and banking them as measured would be mistakes #28 by a new route. A source guard now fails any future `already reported` WATCH that hand-rolls `print`, and `EMITTERS` in `test_qa_early_returns.py` learned the new name so the early-return derivation keeps bounding the two returns it already covered.
...
**PROMOTED, with `--defer=hyxlab-stream.service`, and the reason is the two deferrals it inherited.** `collector/qa.py` runs from the STABLE tree (the unit's `ExecStart`), so unlike 09-18 08:40Z's guard this fix is INERT until promoted -- it had to ship. But `CHANGED` also carried 09-18 02:15Z's `hyxlab/streamstore.py` and `collector/streamd.py`, both in stream's closure, for a change that pass MEASURED as zero behaviour change (comments plus one constant only tests read). Restarting stream to load nothing, mid-panel, is the needless restart EXP-961 prices in days -- deferred, and recorded here. `hyxlab-shadow` was auto-deferred by the guard shipped six hours ago, reading `panel_shortfall 5 10`: **its first production firing, and it fired on the promote it was written for.**
